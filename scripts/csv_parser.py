...
class FlightDataAnalyzer():
  ...
  def plot_current_state(self):
    ...
    ax.plot(time, state, color='brown', linestyle='-', linewidth=2, marker='o', markersize=4, label='Current State')
  ...
  def plot_altitude(self):
    ...
    ax.plot(time, acc_altitude, color='green', linestyle='-', linewidth=2, label='Accelerometer Altitude')
    ax.plot(time, baro_altitude, color='blue', linestyle='-', linewidth=2, label='Barometric Altitude')
    
    # Add horizontal line for maximum altitude
    ax.axhline(y=max_altitude, color='red', linestyle='--', linewidth=2, label='Maximum Altitude')
    
    # Annotate the maximum altitude value
    ax.annotate(f'Max: {max_altitude:.2f} m', 
                xy=(max_altitude_time, max_altitude),
    ...
    ax.set_title('Altitude Over Time', fontsize=14)
    ax.legend(fontsize=11)
    ax.grid(True, alpha=0.3)
  ...
  def plot_motor_data(self):
    """Plot motor target and actual degrees over time"""
    time = self.df['time_s']
    ...
    ax.plot(time, motor_target, color='blue', linestyle='-', linewidth=2, label='Motor Target Degrees')
    ax.plot(time, motor_actual, color='red', linestyle='-', linewidth=2, label='Motor Actual Degrees')
    
    ax.set_xlabel('Time (s)', fontsize=12)
    ax.set_ylabel('Degrees', fontsize=12)
    ax.set_title('Motor Target vs Actual Degrees Over Time', fontsize=14)
    ax.legend(fontsize=11)
    ax.grid(True, alpha=0.3)
  ...
  def plot_motor_and_pressure_data(self):
    ...
    ax1.plot(time, motor_target, color='blue', linestyle='-', linewidth=2, label='Motor Target Degrees')
    ax1.plot(time, motor_actual, color='red', linestyle='-', linewidth=2, label='Motor Actual Degrees')
  ...
  def plot_motor_and_acceleration_data(self):
    ...
    ax2 = ax1.twinx()
    ax2.plot(time, accel_x, color='orange', linestyle='-', linewidth=1.5, label='Accel X')
    ax2.plot(time, accel_y, color='green', linestyle='-', linewidth=1.5, label='Accel Y')
    ax2.plot(time, accel_z, color='purple', linestyle='-', linewidth=1.5, label='Accel Z')
    ax2.set_ylabel('Acceleration (m/s²)', fontsize=12, color='black')
...
def main():
  # Analyze epic_data.csv for time range 1500-1700 seconds
  flight_analyzer = FlightDataAnalyzer('data/epic_data.csv', time_range=(1500, 1700))
