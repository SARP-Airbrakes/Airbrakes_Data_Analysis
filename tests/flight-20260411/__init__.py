...
class FlightDataAnalyzer():
    ...
    def _setup_output_directory(self, file_path, time_range):
        """Create output directory under generated/ based on the data filename"""
        file_name = Path(file_path).stem
        
        # Add time range suffix if filtering
        if time_range is not None:
            file_name = f"{int(time_range[0])}-{int(time_range[1])}"
        
        output_dir = Path('generated/flight-20260411') / file_name
